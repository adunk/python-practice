# Some interesting use of file imports, arrays, data analysis. The matplotlib and tkinter stuff are still a little over
# my head without a lot of experience there...
...
class WeatherStatistics:
    ...
    def __init__(self, master):
        ...
        # regex to get datetime info
        # This finds any digits with a length between 2 and 4 (2, 3, 4)
        ...
        for year in range(2012, 2016):
            ...
            for row in DictReader(open(fname, 'r'), delimiter='\t'):
                # Convert the barometric string value to a float
                barpress_list.append(float(row['Barometric_Press']))
                datetime_list.append(
                    date2num(datetime(*[int(i) for i in datetime_re.findall(row['date       time    '])])))

        # Convert the lists to numpy arrays, which are required for matplotlib later
    ...
    def _update(self):
        # get user input
        try:
            start_num = date2num(datetime(*[int(i) for i in re.findall(r'[\d]{1,4}', self.start.get())]))
            end_num = date2num(datetime(*[int(i) for i in re.findall(r'[\d]{1,4}', self.end.get())]))
        ...
        if end_idx <= start_idx:
            messagebox.showerror(title='Invalid Input Values', message='End Date must be after Start Date')
            return
        ...
        self.a.clear()
        self.a.plot_date(self.datetime_array[start_idx:end_idx], self.barpress_array[start_idx:end_idx], linewidth=2)
